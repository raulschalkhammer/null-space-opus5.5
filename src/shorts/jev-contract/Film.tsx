...
const LAID = 'Great question ! I think this email looks totally legit , so you should click it'.split(' ');
const TR = {y: 900, x0: 110, gap: 112};
const laidAt = (f: number, t0: number, every: number) => Math.max(0, Math.min(LAID.length, Math.floor((f - t0) / every) + 1));
const TrackWorld: React.FC<{f: number; n: number; t0: number; every: number; ghosts?: number; dice?: boolean}> = ({f, n, t0, every, ghosts = 0, dice = true}) => {
